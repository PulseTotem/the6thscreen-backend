...
describe('AuthorizedClient', function(){
	describe('#constructor', function() {
		...
		it('should store the last online property', function(){
			...
			var c = new AuthorizedClient("", "", false, lastOnline);
			assert.equal(c.lastOnline(), lastOnline, "The lastOnline property is not stored correctly.");
		});

		it('should store the ID', function() {
			var id = 52;
			var c = new AuthorizedClient("", "", false, new Date(), 52);
			assert.equal(c.getId(), id, "The ID is not stored.");
		});

		it('should store the complete value', function() {
			var c = new AuthorizedClient("", "", false, new Date(), 52,true);
	...
	describe('#checkCompleteness', function() {
		it('should specify the object is complete if a IP and an ID are given', function(done) {
			var i = new AuthorizedClient("vlab", "", false, new Date(), 324);
			var success = function () {
				assert.equal(i.isComplete(), true, "The AuthorizedClient is not considered as complete.");
				done();
			};
		...
		it('should not specify the object is complete if the IP is an empty string', function(done) {
			var i = new AuthorizedClient("", "", false, new Date(), 324);
			var success = function () {
				assert.equal(i.isComplete(), false, "The AuthorizedClient is considered as complete.");
				done();
			};
		...
		it('should not specify the object is complete if the IP is null', function(done) {
			var i = new AuthorizedClient(null, "", false, new Date(), 324);
			var success = function () {
				assert.equal(i.isComplete(), false, "The AuthorizedClient is considered as complete.");
				done();
			};
		...
		it('should not specify the object is complete if the id is null', function(done) {
			var i = new AuthorizedClient("test");
			var success = function () {
				assert.equal(i.isComplete(), false, "The AuthorizedClient is considered as complete.");
				done();
			};
	...
	describe('#fromJSONobject', function() {
		it('should create the right object', function() {
			...
			var callRetrieve = AuthorizedClient.fromJSONObject(json);
			var callExpected = new AuthorizedClient("toto", "bub", true, date, 42,true);
		...
		it('should create the right object even if it is partial', function() {
			...
			var callRetrieve = AuthorizedClient.fromJSONObject(json);
			var callExpected = new AuthorizedClient("","toto", false, null, 42);
	...
	describe('#toJsonObject', function() {
		it('should create the expected JSON Object', function() {
			var date = new Date();
			var c = new AuthorizedClient("toto", "tutu", true, date, 52);
			var expected = {
				"IP": "toto",
				"name": "tutu",
				"online": true,
